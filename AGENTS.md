...
- `pbp.js` (`window.PBP`) is the play-by-play reconstruction library: plays attributed by snap time against the game clock; empirically validated against full 2025 weeks via `probe.html` (dev page) under a strict criterion — exact TDs/INTs/receptions/fumbles/kicks per player, ≤2 pts yardage drift, no phantom credits. Keep `probe.html` PASSing when touching `pbp.js`.
